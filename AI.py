...
def wishme():
    ...
    if hour >= 0 and hour < 12:
        lisvar.set("Good Morning Sir!")
        tkWindow.update()
        speak("Good Morning Sir!")
    elif hour >= 12 and hour < 18:
        lisvar.set("Good Afternoon Sir!")
        tkWindow.update()
        speak("Good Afternoon Sir!")
    else:
        lisvar.set("Good Evening Sir!")
        tkWindow.update()
...
if __name__ == '__main__':
    ...
    ftop = Frame(tkWindow, bg='#B5B1B1')  # B5B1B1
    ...
    f1 = Frame(tkWindow, bg='#B5b1b1')  # #B5B1B1 highlightbackground="black", highlightthickness=1
    ...
    button.pack()
    # button.place(x=130,y=60,width="250",height=230)

    lisvar = StringVar()
    lis = Label(f1, textvariable=lisvar, font="Century 15 normal", bg='#B5b1b1', fg='black', wraplength=500,
                justify='center')  # B5b1b1
    lisvar.set("Welcome")

    # lisvar.set("Google LLC is an American multinational technology company that specializes in Internet-related services and products, which include online advertising technologies, a search engine, cloud computing, software, and hardware.")
    lis.pack(pady=30)
